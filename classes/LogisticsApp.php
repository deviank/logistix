...
class LogisticsApp {
    ...
    public function generateGameIdea() {
        echo json_encode([
            'success' => true,
            'idea' => $this->buildGameIdea()
        ]);
    }

    private function buildGameIdea() {
        $data = $this->getGameIdeaGeneratorData();

        $genre = $this->randomFrom($data['genres']);
        $theme = $this->randomFrom($data['themes']);
        $coreAction = $this->randomFrom($data['core_actions']);
        $progression = $this->randomFrom($data['progression_loops']);
        $reward = $this->randomFrom($data['variable_rewards']);
        $socialHook = $this->randomFrom($data['social_hooks']);
        $mastery = $this->randomFrom($data['mastery_axes']);
        $metaSystem = $this->randomFrom($data['meta_systems']);
        $sessionHook = $this->randomFrom($data['session_hooks']);
        $twist = $this->randomFrom($data['twists']);

        $titlePrefix = $this->randomFrom($data['title_prefixes']);
        $titleSuffix = $this->randomFrom($data['title_suffixes']);
        $title = $titlePrefix . ' ' . $titleSuffix;

        $coreLoop = [
            "Scout " . strtolower($theme) . " opportunities in short " . strtolower($sessionHook) . " runs.",
            $coreAction . " to earn resources and improve your " . strtolower($genre) . " toolkit.",
            "Cash in rewards to unlock " . strtolower($progression) . " upgrades, then raise the stakes."
        ];

        $compulsionElements = [
            'Onboarding Hook' => "First 60 seconds delivers one clear win: " . $coreAction . ".",
            'Progression Arc' => ucfirst($progression) . " ensures players always have a visible next goal.",
            'Variable Reward' => $reward . " keeps outcomes surprising instead of predictable.",
            'Social Pressure' => $socialHook . " creates status, rivalry, or cooperation tension.",
            'Mastery Depth' => $mastery . " gives skilled players room to optimize and improve."
        ];

        $retentionSafeguards = $this->randomSample($data['retention_safeguards'], 3);

        return [
            'title' => $title,
            'genre' => $genre,
            'theme' => $theme,
            'one_liner' => "A " . strtolower($genre) . " where players " . strtolower($coreAction) . " inside a " . strtolower($theme) . " world and chase " . strtolower($metaSystem) . ".",
            'core_loop' => $coreLoop,
            'compulsion_elements' => $compulsionElements,
            'signature_system' => $twist,
            'meta_system' => $metaSystem,
            'retention_safeguards' => $retentionSafeguards
        ];
    }

    private function getGameIdeaGeneratorData() {
        return [
            'genres' => [
                'Action Roguelite',
                'Cozy Strategy',
                ...
                'Deckbuilder RPG',
                'Automation Sandbox',
                'Social Deduction Adventure',
                'Rhythm Combat'
            ],
            'themes' => [
                'floating sky-city frontier',
                'post-collapse megamall',
                'mythic ocean kingdom',
                'time-looping desert colony',
                'haunted biotech campus',
                'underground neon metropolis',
                'living clockwork forest',
                'volcanic island archipelago'
            ],
            'core_actions' => [
                'chain high-risk combos',
                'build and defend improvised bases',
                'draft synergistic ability cards',
                'outsmart rival crews in short heists',
                'optimize production routes under pressure',
                'negotiate fragile alliances for scarce loot',
                'hunt elite targets with evolving move-sets',
                'salvage relics before hazards escalate'
            ],
            'progression_loops' => [
                'account-wide unlock trees',
                'branching character class evolutions',
                'base-building milestones',
                'seasonal faction reputation tracks',
                'run-to-run permanent upgrades',
                'crafting mastery tiers',
                'prestige resets with power multipliers',
                'collection milestones with set bonuses'
            ],
            'variable_rewards' => [
                'procedural loot with rare mutators',
                'mystery events that remix objectives',
                'dynamic enemy behavior that changes each run',
                'hidden quest chains triggered by player style',
                'risk-based payout chests with escalating odds',
                'rotating world modifiers that alter strategy'
            ],
            'social_hooks' => [
                'weekly guild raids with shared rewards',
                'ghost replays of top players to challenge',
                'asynchronous market economy for crafted gear',
                'clan-versus-clan territory pushes',
                'co-op rescues that can pivot into betrayal',
                'spectator voting that changes live match rules'
            ],
            'mastery_axes' => [
                'animation-cancel timing and positioning',
                'resource routing efficiency',
                'team composition synergy planning',
                'prediction of enemy pattern branches',
                'bluffing and social reads',
                'precision rhythm windows for damage boosts'
            ],
            'meta_systems' => [
                'legend reputation',
                'base customization trophies',
                'hero bond stories',
                'season ladder rank',
                'artifact museum completion',
                'faction influence dominance'
            ],
            'session_hooks' => [
                '10-minute',
                '15-minute',
            ...
            'twists' => [
                'Every third run, players vote on a global rule change that affects everyone for 24 hours.',
                'Bosses learn from your previous build choices and counter them on rematches.',
                'The map is player-built over time, so each community shard has unique geography.',
                'Failure still grants blueprint fragments, making every run feel productive.',
                'Alliances can be formalized with contracts that both sides can strategically break.',
                'Music tempo shifts become a combat mechanic that buffs or nerfs entire teams.'
            ],
            'retention_safeguards' => [
                'Daily goals cap at 20 minutes to reduce burnout.',
                'Bad-luck protection gradually increases rare drop odds.',
                'Comeback mechanics prevent early losses from deciding the full match.',
                'Optional chill mode offers progression without ranked pressure.',
                'Flexible party scaling keeps mixed-skill friend groups viable.',
                'Transparent reward probabilities build trust with players.'
            ],
            'title_prefixes' => [
                'Neon',
                'Echo',
    ...
    private function randomSample(array $items, $count) {
        shuffle($items);
